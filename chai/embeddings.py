...
def create_or_search_embeddings(
    ...
    if chroma_path.exists():
        logging.info("Loading persisted embeddings...")
        client = PersistentClient(
            path=path, settings=Settings(anonymized_telemetry=False)
        )
        collection = client.get_collection("langchain")
        query_result = collection.query(
            query_embeddings=openai_embeddings.embed_query(query), n_results=n_results
        )
        ...
        result = db.similarity_search(query=query, k=n_results)
...
def generate_recommendations(query: str = "", reviews: str = "") -> str:
    template = """
    You are an expert in alternative music.

    Based on the Pitchfork reviews below, suggest me a music
    album that is similar to the band or artist "{query}".
    ...
    Include info, like for instance, when this album was released, 
    which musical style is it, and also the review score.

    ANSWER:
    """

    prompt = PromptTemplate(input_variables=["reviews", "query"], template=template)
    llm = OpenAI(temperature=0.7, max_tokens=512)
    llm_chain = LLMChain(llm=llm, prompt=prompt, verbose=True)

    response = llm_chain.run(query=query, reviews=reviews)
...
if __name__ == "__main__":
    df = get_pandas_df()
    query = input()
    docs = create_or_search_embeddings(texts=list(df.values), query=query, n_results=10)
